...
async function register(params, origin) {
    ...
    const token = jwt.sign({ id: account.id }, config.secret, { expiresIn: '1d' });

    // Send email
    await sendVerificationEmail(account, token, origin);
}
...
async function forgotPassword({ email }, origin) {
    ...
    // Create a reset token that expires after 24 hours
